...
var _defineProperty = require("@babel/runtime/helpers/defineProperty"), pLazy = require("p-lazy"), pReflect = require("p-reflect"), isPromise = require("p-is-promise"), semver = require("semver");
...
var _defineProperty__default = _interopDefault(_defineProperty), pLazy__default = _interopDefault(pLazy), pReflect__default = _interopDefault(pReflect), isPromise__default = _interopDefault(isPromise), semver__default = _interopDefault(semver);
...
const noop = x => x, identity = noop, getType = thing => Object.prototype.toString.call(thing).replace(/\[object (.*)\]/, "$1"), escapeRegExp = str => (str || "").replace(/[\-\[\]\/\{\}\(\)\*\+\?\.\\\^\$\|]/g, "\\$&"), mapKeys = (obj, func) => Object.entries(obj).reduce((acc, [key, value]) => _objectSpread(_objectSpread({}, acc), {}, {
  ...
  const returnValue = {}, errors = {}, allPromises = Object.keys(obj).map(key => pReflect__default.default(obj[key]).then(val => (val.isFulfilled ? returnValue[key] = val.value : val.isRejected && (errors[key] = val.reason), 
  ...
  return {
    promise: new pLazy__default.default((resolve, reject) => {
  ...
  const values = [], promises = executors.map(executor => {
    try {
      ...
      if (!isPromise__default.default(loadingPromiseOrError)) throw loadingPromiseOrError;
  ...
  semver__default.default.coerce(input)), v1 = parseVersion(comp), v2 = parseVersion(base);
